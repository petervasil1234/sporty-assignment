...
public class EventOutcomeKafkaProducer {
    ...
    public void send(EventOutcomeMessage message) {
        try {
            kafkaTemplate
                    .send(KafkaTopics.EVENT_OUTCOMES, message.eventId(), message)
                    .get();
            log.debug("Published to topic: {} eventId: {}", KafkaTopics.EVENT_OUTCOMES, message.eventId());
        } catch (ExecutionException | InterruptedException e) {
            log.error("Failed to publish to topic: {} eventId: {}", KafkaTopics.EVENT_OUTCOMES, message.eventId(), e);
            throw new KafkaException("Failed to publish to topic: " + KafkaTopics.EVENT_OUTCOMES, e);
        }
    }
}
